...
オプションは CLI フラグとしてこのページに記載されていますが、設定ファイルオプションとしても渡すことができます。YAML 設定ファイルの使用については、link:../installation/configuration.adoc#configuration-file[設定ファイル]のドキュメントを参照してください。
...
| `--disable` value
| "link:../installation/packaged-components.adoc#using-the---disable-flag[``--disable``フラグの使用]" を参照
...
| `--disable-agent`
| "link:../advanced.adoc#running-agentless-servers-experimental[エージェントレスサーバーの実行（実験的）]" を参照
