...
オプションはCLIフラグとしてこのページに記載されていますが、設定ファイルオプションとしても渡すことができます。YAML設定ファイルの使用方法については、link:../installation/configuration.adoc#configuration-file[設定ファイル]のドキュメントを参照してください。
...
| "link:../installation/private-registry.adoc#default-endpoint-fallback[デフォルトエンドポイントフォールバック]"を参照
|===
