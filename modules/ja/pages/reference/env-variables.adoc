...
| `INSTALL_K3S_EXEC`
| サービスでK3sを起動するためのフラグ付きコマンド。コマンドが指定されておらず、``K3S_URL``が設定されている場合、デフォルトは「agent」です。``K3S_URL``が設定されていない場合、デフォルトは「server」です。詳細はlink:../installation/configuration.adoc#configuration-with-install-script[この例]を参照してください。
