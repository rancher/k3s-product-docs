...
他のすべての CLI フラグと同様に、link:configuration.adoc#configuration-file[設定ファイル] を使用してコンポーネントを無効にすることができます。例えば、専用の `etcd` ノードを作成するには、次の値を `/etc/rancher/k3s/config.yaml` に配置します:
