...
有关这些组件如何协同工作的详细信息，请参阅xref:../architecture.adoc#_高可用-k3s[架构]。
...
根据数据库类型的不同，数据存储端点的格式也不同。有关详细信息，请参阅xref:../datastore/datastore.adoc#_数据存储端点格式和功能[数据存储端点格式]。

要在启动 server 节点时配置 TLS 证书，请参阅xref:../datastore/datastore.adoc#_外部数据库配置参数[数据存储配置指南]。
