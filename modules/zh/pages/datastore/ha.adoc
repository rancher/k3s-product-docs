...
有关这些组件如何协同工作的详细信息，请参阅xref:../architecture.adoc#高可用-k3s[架构]。
...
例如，你可以使用如下命令安装 K3s Server，并使用 MySQL 数据库作为外部数据存储和xref:../cli/server.adoc#集群选项[设置 token]：
...
根据数据库类型的不同，数据存储端点的格式也不同。有关详细信息，请参阅xref:../datastore/datastore.adoc#数据存储端点格式和功能[数据存储端点格式]。

要在启动 server 节点时配置 TLS 证书，请参阅xref:../datastore/datastore.adoc#外部数据库配置参数[数据存储配置指南]。
...
然后可以xref:../cli/server.adoc#集群选项[使用 Token] 添加其他 Server 节点：
