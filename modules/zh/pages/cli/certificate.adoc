...
权威 CA 证书和密钥存储在数据存储区的引导程序密钥中，使用 link:token.adoc#server[server token] 作为 PBKDF2 密码和 AES256-GCM 和 HMAC-SHA1 进行加密。
...
如果使用新的根 CA，则需要中断才能进行轮换。必须使用 `k3s certificate rotate-ca --force` 选项，所有使用 link:token.adoc#secure[secure token] 加入的节点（包括 server）都需要重新配置才能使用新的 Token 值，并且 Pod 需要重启才能信任新的根 CA。
...
如果你使用了 `--force` 选项或更改了根 CA，请确保使用 link:token.adoc#secure[secure token] 加入的节点在重启前都重新配置为使用新的 Token 值。
...
如果集群已使用默认的自签名 CA 证书启动，轮换将是中断的。使用 link:token.adoc#secure[secure token] 加入的所有节点都需要重新配置才能信任新的 CA 哈希。
...
请确保使用 link:token.adoc#secure[secure token] 加入的节点（包括 server 节点）在重启前都重新配置为使用新的 Token 值。
