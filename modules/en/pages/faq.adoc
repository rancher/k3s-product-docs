...
See link:./advanced.adoc#additional-logging-sources[Additional Logging Sources] for even more log options.

== Can I run K3s in Docker?

Yes, there are multiple ways to run K3s in Docker. See link:./advanced.adoc#running-k3s-in-docker[Advanced Options] for more details.
...
2) Check that you have resolved any link:./installation/requirements.adoc#operating-systems[Additional OS Preparation]. Run `k3s check-config` and ensure that it passes.
