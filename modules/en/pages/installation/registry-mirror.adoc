...
Images imported via xref:./airgap.adoc#manually-deploy-images-method[air-gap image tar files] are pinned in containerd to
...
and only pull images from the configured mirrors and/or the embedded mirror, see the xref:./private-registry.adoc#default-endpoint-fallback[Default Endpoint Fallback]
