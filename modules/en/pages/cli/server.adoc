...
Options are documented on this page as CLI flags, but can also be passed as configuration file options. See the xref:../installation/configuration.adoc#configuration-file[Configuration File] documentation for more information on using YAML configuration files.
...
| `--disable` value
| See "xref:../installation/packaged-components.adoc#using-the---disable-flag[Using the `--disable` flag]"
...
| `--disable-agent`
| See "xref:../advanced.adoc#running-agentless-servers-experimental[Running Agentless Servers (Experimental)]"
