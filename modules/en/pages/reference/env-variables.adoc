...
| `INSTALL_K3S_EXEC`
| Command with flags to use for launching K3s in the service. If the command is not specified, and the `K3S_URL` is set, it will default to "agent." If `K3S_URL` not set, it will default to "server." For help, refer to link:../installation/configuration.adoc#configuration-with-install-script[this example.]
