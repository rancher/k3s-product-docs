...
For information on using custom CA certificates, or renewing the self-signed CA certificates, see the link:./cli/certificate.adoc#certificate-authority-ca-certificates[`k3s certificate rotate-ca` command documentation].
...
For information on manually rotating client and server certificates, see the link:./cli/certificate.adoc#client-and-server-certificates[`k3s certificate rotate` command documentation].
...
K3s agents can be configured with the options `--node-label` and `--node-taint` which adds a label and taint to the kubelet. The two options only add labels and/or taints link:./cli/agent.adoc#node-labels-and-taints-for-agents[at registration time], so they can only be set when the node is first joined to the cluster.
...
This option can also be specified in the K3s link:./installation/configuration.adoc#configuration-file[configuration file].
