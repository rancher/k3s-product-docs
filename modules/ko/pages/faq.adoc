...
더 많은 로그 옵션은 xref:./advanced.adoc#additional-logging-sources[추가 로깅 소스]를 참조하세요.

== Docker에서 K3s를 실행할 수 있나요?

예, Docker에서 K3s를 실행하는 방법은 여러 가지가 있습니다. 자세한 내용은 xref:./advanced.adoc#running-k3s-in-docker[고급 옵션]을 참조하세요.
...
. xref:./advanced.adoc#추가-os-준비-사항[추가 OS 준비사항]을 모두 해결했는지 확인합니다. ``k3s check-config``를 실행하고 통과했는지 확인합니다.
