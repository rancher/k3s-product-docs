...
The default mode is `agent`. `pod` or `cluster` modes are recommended when running link:../advanced.adoc#에이전트-없는-서버-실행하기실험적[agentless servers], in order to provide the apiserver with access to cluster service endpoints in the absence of flannel and kube-proxy.
