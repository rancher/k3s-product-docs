 ...
 ** K3s は、クラスター CA 証明書が既存のルートまたは中間 CA によって署名されている場合に正しく機能するようになりました。K3s が開始する前にそのような証明書を生成するためのサンプルスクリプトは、github リポジトリの https://github.com/k3s-io/k3s/blob/main/contrib/util/certs.sh[contrib/util/certs.sh] にあります。
...
このリリースは、v1.24ブランチに``--docker``フラグの使用を復元します。詳細はlink:https://github.com/k3s-io/k3s/blob/main/docs/adrs/cri-dockerd.md[docs/adrs/cri-dockerd.md]を参照してください。
