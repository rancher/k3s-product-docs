...
適切な証明書とキーを事前に作成するためのサンプルスクリプトは、https://github.com/k3s-io/k3s/blob/main/contrib/util/generate-custom-ca-certs.sh[K3sリポジトリの``contrib/util/generate-custom-ca-certs.sh``]にあります。
...
既存のCAによってクロス署名された更新されたCA証明書とキーを作成するためのサンプルスクリプトは、https://github.com/k3s-io/k3s/blob/main/contrib/util/rotate-default-ca-certs.sh[K3sリポジトリの``contrib/util/rotate-default-ca-certs.sh``]にあります。
