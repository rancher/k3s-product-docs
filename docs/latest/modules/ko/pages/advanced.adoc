...
* `+https://github.com/k3s-io/k3s/blob/<VERSION>/k3s-rootless.service+`](https://github.com/k3s-io/k3s/blob/main/k3s-rootless.service)에서 ``k3s-rootless.service``를 다운로드한다.
...
K3s용 https://documentation.suse.com/cloudnative/rancher-manager/latest/en/observability/logging/logging-helm-chart-options.html[랜처 로깅]은 랜처를 사용하지 않고 설치할 수 있습니다. 이를 위해서는 다음 지침을 실행해야 합니다:
