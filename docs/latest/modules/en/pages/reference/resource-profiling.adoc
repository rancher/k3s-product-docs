= Resource Profiling
:description: This page determines minimum resource requirements for K3s by testing CPU, memory, and IOPS needed to run the K3s stack, monitoring tools, and example workloads
...
[#_minimum_resource_requirements_for_k3s]
...
[#_components_included_for_baseline_measurements]
...
[#_baseline_resource_requirements]
...
[#_k3s_cluster_with_a_single_agent]
==== K3s Cluster with a Single Agent
...
[#_analysis_of_primary_resource_utilization_drivers]
...
[#_preventing_agents_and_workloads_from_interfering_with_the_cluster_datastore]
...
[#_a_note_on_high_availability_ha]
