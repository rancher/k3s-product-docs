= CIS Hardening Guide
:description: Follow steps in the CIS Hardening Guide to configure K3s, set kernel parameters, and apply security controls to meet CIS Benchmark requirements
:revdate: 2026-05-29
...
[#_ensure_protect_kernel_defaults_is_set]
...
[#_configuration_for_kubernetes_components]
...
If you have followed this guide, your K3s cluster will be configured to comply with the CIS Kubernetes Benchmark. You can review the xref:security/self-assessment-1.12.adoc[CIS 1.12 Self-Assessment Guide] to understand the expectations of each of the benchmark's checks and how you can do the same on your cluster.
