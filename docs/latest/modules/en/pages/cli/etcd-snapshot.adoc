= k3s etcd-snapshot
:description: Manage etcd snapshots with the k3s etcd-snapshot CLI tool by creating, deleting, and restoring scheduled or on-demand backups stored on the node's file system
:revdate: 2026-07-24
...
If xref:cli/secrets-encrypt.adoc[secrets encryption] is enabled, snapshots also include the secrets encryption configuration and keys.
