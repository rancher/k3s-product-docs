= k3s server
:description: Configure the K3s server using CLI flags or configuration files to set critical options for cluster operation
:revdate: 2026-03-31
...
| `--etcd-s3-bucket-lookup-type` value
|
|
...
| Default local storage path for local provisioner storage class (default: `/var/lib/rancher/k3s/storage`)
|===
...
[#_customized_flags_for_kubernetes_processes]
