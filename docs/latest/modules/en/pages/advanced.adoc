...
:description: Configure K3s advanced options for certificate management, token rotation, DNS resolution, and HTTP proxy settings
...
[#_certificate_authority_certificates]
...
[#_configuring_dns_resolution]
== Configuring DNS Resolution

[#_nameserver_viability_checks]
...
K3s performs these checks because nameserver addresses of these types would not function properly within pods that https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy[inherit name resolution configuration] from their node.
If no usable resolv.conf is found, K3s prints a warning message to the logs, and generates a stub resolver configuration that uses `8.8.8.8` and `2001:4860:4860::8888` as the nameservers.
...
[#_coredns_custom_configuration_imports]
...
[#_using_docker_as_the_container_runtime]
...
[#_alternative_container_runtime_support]
...
[#_running_rootless_servers_experimental]
...
[#_advanced_rootless_configuration]
...
[#_starting_the_service_with_the_installation_script]
...
[#_enabling_lazy_pulling_of_estargz_experimental]
== Enabling Lazy Pulling of eStargz (Experimental)

[#_whats_lazy_pulling_and_estargz]
...
[#_configure_k3s_for_lazy_pulling_of_estargz]
...
[#_additional_network_policy_logging]
