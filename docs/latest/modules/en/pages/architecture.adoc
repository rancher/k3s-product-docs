= Architecture
:description: Learn about K3s architecture, including server and agent node roles, single-server and high-availability configurations, agent registration, load balancer
:revdate: 2026-04-01
...
[#_single_server_setup_with_an_embedded_db]
...
[#_fixed_registration_address_for_agent_nodes]
...
When connected to the cluster, the agent retrieves a list of kube-apiserver addresses from the Kubernetes service endpoint list in the default namespace.
