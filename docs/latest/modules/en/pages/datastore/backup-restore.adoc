= Backup and Restore
:description: Backup and restore operations for K3s depend on the type of datastore used, requiring copying specific directories for SQLite and handling external databases
...
[#_backup_and_restore_with_embedded_etcd_datastore]
