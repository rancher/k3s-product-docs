...
不要手动编辑 `traefik.yaml` 文件，因为 K3s 会在启动时使用默认值替换该文件。相反，你需要通过在 `/var/lib/rancher/k3s/server/manifests` 中创建其他 `HelmChartConfig` 清单来自定义 Traefik。有关更多详细信息和示例，请参阅xref:add-ons/helm.adoc#_customizing_packaged_components_with_helmchartconfig[使用 HelmChartConfig 自定义打包组件]。有关配置值的更多信息，请参阅link:https://github.com/traefik/traefik-helm-chart/tree/master/traefik[官方 Traefik Helm 配置参数]。
...
要从较旧的 Traefik v1 实例迁移，请参阅 https://doc.traefik.io/traefik/migrate/v1-to-v2/[Traefik 文档]和link:https://github.com/traefik/traefik-migration-tool[迁移工具]。

== 网络策略控制器

K3s 包含一个嵌入式网络策略控制器。底层实现是 https://github.com/cloudnativelabs/kube-router[kube-router] 的 netpol 控制器库（没有其他 kube-router 功能），你可以在link:https://github.com/k3s-io/k3s/tree/main/pkg/agent/netpol[此处]找到 netpol。
