...
** Download the artifacts and run the xref:installation/airgap.adoc#_2_install_k3s[install script] locally. Add the environment variable `INSTALL_K3S_SKIP_START="true"` when running the install script to prevent K3s from starting.
...
** Download the artifacts and run the xref:installation/airgap.adoc#_2_install_k3s[install script] locally. Verify the K3s version after install with `k3s --version` and reapply any custom configurations that where used before the upgrade. 
